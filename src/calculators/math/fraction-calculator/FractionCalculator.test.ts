...
import { validateFractionCalculatorInputs } from './validation';
...
describe('Fraction Calculator', () => {
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateFractionCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates negative numbers', () => {
      const invalidInputs = { ...mockInputs, inputValue: -5 };
      const result = validateFractionCalculatorInputs(invalidInputs);

...
import { validateTrigonometryCalculatorInputs } from './validation';
...
describe('Trigonometry Calculator', () => {
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateTrigonometryCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates negative numbers', () => {
      const invalidInputs = { ...mockInputs, inputValue: -5 };
      const result = validateTrigonometryCalculatorInputs(invalidInputs);

...
import { validateMatrixCalculatorInputs } from './validation';
...
describe('Matrix Calculator', () => {
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateMatrixCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates negative numbers', () => {
      const invalidInputs = { ...mockInputs, inputValue: -5 };
      const result = validateMatrixCalculatorInputs(invalidInputs);

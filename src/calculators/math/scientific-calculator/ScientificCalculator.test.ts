...
import { validateScientificCalculatorInputs } from './validation';
...
describe('Scientific Calculator', () => {
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateScientificCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates negative numbers', () => {
      const invalidInputs = { ...mockInputs, inputValue: -5 };
      const result = validateScientificCalculatorInputs(invalidInputs);

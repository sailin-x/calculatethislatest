...
describe('Required Beginning Date (RBD) for RMDs Calculator', () => {
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateRequiredBeginningDateRbdForRmdsCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates negative numbers', () => {
      const invalidInputs = { ...mockInputs, inputValue: -5 };
      const result = validateRequiredBeginningDateRbdForRmdsCalculatorInputs(invalidInputs);

...
import { validateCrowdfundingEquityOfferingInputs } from './validation';
...
describe('Crowdfunding Equity Offering Calculator', () => {
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateCrowdfundingEquityOfferingInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates funding goal cannot be zero', () => {
      const invalidInputs = { ...mockInputs, totalFundingGoal: 0 };
      const result = validateCrowdfundingEquityOfferingInputs(invalidInputs);

import { CrowdfundingEquityOfferingInputs, CrowdfundingEquityOfferingOutputs, CrowdfundingEquityOfferingMetrics, CrowdfundingEquityOfferingAnalysis } from './types';

// Calculate total equity offered as percentage
export function calculateTotalEquityOffered(inputs: CrowdfundingEquityOfferingInputs): number {
  return inputs.equityPercentageOffered;
}

// Calculate price per share based on valuation and equity offered
export function calculatePricePerShare(inputs: CrowdfundingEquityOfferingInputs): number {
...
export function calculateSharesOffered(inputs: CrowdfundingEquityOfferingInputs): number {
...
export function calculateFundingProgress(inputs: CrowdfundingEquityOfferingInputs): number {
  return (inputs.currentFunding / inputs.totalFundingGoal) * 100;
}

// Calculate remaining funding needed
export function calculateRemainingFundingNeeded(inputs: CrowdfundingEquityOfferingInputs): number {
  return Math.max(0, inputs.totalFundingGoal - inputs.currentFunding);
}

// Calculate average investment per investor
export function calculateAverageInvestmentPerInvestor(inputs: CrowdfundingEquityOfferingInputs): number {
  return inputs.numberOfInvestors > 0 ? inputs.currentFunding / inputs.numberOfInvestors : 0;
}

// Calculate total fees
export function calculateTotalFees(inputs: CrowdfundingEquityOfferingInputs): number {
  return inputs.platformFees + inputs.legalFees + inputs.marketingFees;
}

// Calculate net proceeds after fees
export function calculateNetProceeds(inputs: CrowdfundingEquityOfferingInputs): number {
  return inputs.currentFunding - calculateTotalFees(inputs);
}

// Calculate post-money valuation
export function calculatePostMoneyValuation(inputs: CrowdfundingEquityOfferingInputs): number {
  return inputs.valuation + inputs.currentFunding;
}

// Calculate ownership dilution
export function calculateOwnershipDilution(inputs: CrowdfundingEquityOfferingInputs): number {
...
export function calculateBreakEvenInvestors(inputs: CrowdfundingEquityOfferingInputs): number {
...
export function calculateProjectedROI(inputs: CrowdfundingEquityOfferingInputs): number {
  return inputs.expectedReturn;
}

// Calculate risk-adjusted return
export function calculateRiskAdjustedReturn(inputs: CrowdfundingEquityOfferingInputs): number {
...
export function calculateSuccessProbability(inputs: CrowdfundingEquityOfferingInputs): number {
...
export function calculateResult(inputs: CrowdfundingEquityOfferingInputs): number {
  return calculateFundingProgress(inputs);
}

// Generate analysis based on inputs and metrics
export function generateAnalysis(inputs: CrowdfundingEquityOfferingInputs, metrics: CrowdfundingEquityOfferingMetrics): CrowdfundingEquityOfferingAnalysis {
  const progress = calculateFundingProgress(inputs);

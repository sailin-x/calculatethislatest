export interface AlphaInputs {
  ...
  // Risk Metrics
  targetAlpha: number; // Target alpha level
  maxTrackingError: number; // Maximum acceptable tracking error
...
export interface AlphaMetrics {
  ...
  alphaTStatistic: number; // T-statistic for alpha
  alphaPValue: number; // P-value for alpha significance
  alphaStandardError: number; // Standard error of alpha
...
export interface AlphaAnalysis {
  // Executive Summary
  alphaRating: 'Excellent' | 'Good' | 'Average' | 'Poor' | 'Very Poor';
...
export interface AlphaOutputs extends AlphaMetrics {

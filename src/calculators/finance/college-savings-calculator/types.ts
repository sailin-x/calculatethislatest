export interface CollegeSavingsInputs {
  ...
  desiredContributionPercentage: number;
  savingsGoal: number;
  ...
  contributionFrequency: 'Monthly' | 'Quarterly' | 'Semi-Annual' | 'Annual';
  ...
  // Tax considerations
  accountType: '529 Plan' | 'Coverdell ESA' | 'UTMA/UGMA' | 'Trust' | 'Taxable Account';
  ...
  filingStatus: 'Joint' | 'Separate' | 'Head of Household';

  // Financial aid considerations
  expectFinancialAid: boolean;
  ...
  savingsKnowledge: 'Expert' | 'Knowledgeable' | 'Basic' | 'Novice';
...
export interface CollegeSavingsOutputs {
  ...
  // Comparative analysis
  vs529Plan: number;
  vsCoverdellESA: number;
  vsTaxableAccount: number;
  vsTrust: number;

  // Account optimization
  optimalAccountType: string;
  ...
  // Recommendation
  overallRecommendation: 'Strong Implement' | 'Implement' | 'Consider Alternatives' | 'Delay Strategy';
  ...
  finalRecommendation: 'Aggressive 529 Plan' | 'Balanced Approach' | 'Conservative Strategy' | 'Alternative Methods';
  implementationConfidence: 'High' | 'Medium' | 'Low';
  strategicConsiderations: string[];
  tacticalActions: string[];

  // Educational content
  collegeSavingsEducation: string[];
  accountTypeEducation: string[];
  ...
  alternativeApproaches: {
    prepaidTuition: number;
    scholarshipFocus: number;
    workStudyEmphasis: number;

...
export function validateDebtYieldInputs(inputs: CalculatorInputs): ValidationResult {
  const errors: string[] = [];
  ...
  const requiredFields = [
    ...
    'accountingFees', 'loanAmount', 'propertyValue', 'requiredDebtYield',
    'marketCapRate', 'propertyAge', 'occupancyRate', 'leaseType',
    'tenantCreditRating', 'marketConditions', 'loanTerm', 'interestRate',
    'amortizationPeriod', 'loanToValue', 'debtServiceCoverageRatio'
  ];
  ...
  if (amortizationPeriod < 1 || amortizationPeriod > 50) {
    errors.push('Amortization period must be between 1 and 50 years');
  }
  ...
  if (requiredDebtYield < 7 || requiredDebtYield > 15) {
    errors.push('Required debt yield must be between 7% and 15%');
  }
  ...
  if (debtServiceCoverageRatio < 0 || debtServiceCoverageRatio > 5) {
    errors.push('Debt service coverage ratio must be between 0 and 5');
  }
  ...
  if (loanTerm > amortizationPeriod) {
...
export function quickValidateDebtYieldInput(field: string, value: any): string | null {
  switch (field) {
    ...
    case 'amortizationPeriod':
      ...
      const amortization = Number(value);
      if (isNaN(amortization)) {
        return 'Amortization period must be a valid number';
      }
      if (amortization < 1 || amortization > 50) {
        return 'Amortization period must be between 1 and 50 years';
      }
      break;
    ...
    case 'requiredDebtYield':
      if (value === undefined || value === null || value === '') {
        return 'Required debt yield is required';
      }
      const debtYield = Number(value);
      if (isNaN(debtYield)) {
        return 'Required debt yield must be a valid number';
      }
      if (debtYield < 7 || debtYield > 15) {
        return 'Required debt yield must be between 7% and 15%';
      }
      break;
    ...
    case 'debtServiceCoverageRatio':
      if (value === undefined || value === null || value === '') {
        return 'Debt service coverage ratio is required';
      }
      ...
      if (dscr < 0 || dscr > 5) {
        return 'Debt service coverage ratio must be between 0 and 5';
      }
      break;
  }

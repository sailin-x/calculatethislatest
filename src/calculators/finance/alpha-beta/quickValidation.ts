...
export function validateMarketVolatility(value: number, allInputs?: Record<string, any>): string | null {
  if (value !== undefined) {
    if (value < 0) {
      return 'Market volatility cannot be negative';
    }
    if (value > 100) {
      return 'Market volatility over 100% seems unrealistic';
    }
  }
  return null;
}

export function validateTargetVolatility(value: number, allInputs?: Record<string, any>): string | null {
  if (value !== undefined) {
    if (value < 0) {
      return 'Target volatility cannot be negative';
    }
    if (value > 50) {
      return 'Target volatility over 50% seems unrealistic';
    }
  }
  return null;
}

export function validateMaxDrawdown(value: number, allInputs?: Record<string, any>): string | null {
  if (value !== undefined) {
    if (value < 0) {
      return 'Maximum drawdown cannot be negative';
    }
    if (value > 100) {
      return 'Maximum drawdown over 100% seems unrealistic';
    }
  }
  return null;
}
...
export function validateCustomBenchmark(value: boolean, allInputs?: Record<string, any>): string | null {
  if (value === true) {
    if (!allInputs?.benchmarkWeights || allInputs.benchmarkWeights.length === 0) {
      return 'Custom benchmark requires benchmark weights';
    }
  }
  return null;
}

export function validateBenchmarkWeights(value: number[], allInputs?: Record<string, any>): string | null {
  if (value && value.length > 0) {
    const sum = value.reduce((total, weight) => total + weight, 0);
    if (Math.abs(sum - 1) > 0.01) {
      return 'Benchmark weights must sum to 1.0';
    }
    
    const negativeWeights = value.filter(w => w < 0);
    if (negativeWeights.length > 0) {
      return 'Benchmark weights cannot be negative';
    }
  }
  return null;
}

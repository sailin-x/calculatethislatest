...
import { AlphaBetaInputs, AlphaBetaOutputs, AlphaBetaMetrics, AlphaBetaAnalysis } from './types';

/**
 * Calculate comprehensive alpha and beta analysis
 */
export function calculateAlphaBeta(inputs: AlphaBetaInputs): AlphaBetaOutputs {
  ...
  const optimizationSuggestions = generateOptimizationSuggestions(inputs, alpha, beta, sharpeRatio, informationRatio);
...
function calculatePValue(tStatistic: number, degreesOfFreedom: number): number {
  ...
  if (absT > 1.645) return 0.1;   // 90% confidence
  return 0.5; // Default for non-significant
}
...
function calculateConfidenceInterval(alpha: number, standardError: number, confidenceLevel: number, degreesOfFreedom: number): { lower: number; upper: number } {
  // Simplified confidence interval using normal distribution
...
function generateAnalysis(inputs: AlphaBetaInputs, metrics: any): AlphaBetaAnalysis {
  ...
  let performanceRating: 'Excellent' | 'Good' | 'Average' | 'Poor' | 'Very Poor' = 'Average';
  ...
  // Risk rating logic
  if (metrics.portfolioVolatility < 10) {
    riskRating = 'Low';
  } else if (metrics.portfolioVolatility < 20) {
    riskRating = 'Moderate';
  } else if (metrics.portfolioVolatility < 30) {
    riskRating = 'High';
  } else {
  ...
  if (performanceRating === 'Excellent') {
    ...
    recommendation = 'Sell';
  } else if (performanceRating === 'Very Poor') {
    recommendation = 'Strong Sell';
  }
  
  // Alpha significance
  let alphaSignificance: 'Highly Significant' | 'Significant' | 'Marginally Significant' | 'Not Significant' = 'Not Significant';
  if (metrics.pValue < 0.01) {
    alphaSignificance = 'Highly Significant';
  } else if (metrics.pValue < 0.05) {
    alphaSignificance = 'Significant';
  } else if (metrics.pValue < 0.1) {
    alphaSignificance = 'Marginally Significant';
  }
  
  // Beta stability (simplified)
  const betaStability: 'Very Stable' | 'Stable' | 'Moderately Stable' | 'Unstable' = 
    metrics.rSquared > 0.8 ? 'Very Stable' : 
    metrics.rSquared > 0.6 ? 'Stable' : 
    metrics.rSquared > 0.4 ? 'Moderately Stable' : 'Unstable';
  ...
  return {
    ...
    performanceSummary: `Portfolio shows ${isPositiveAlpha ? 'positive' : 'negative'} alpha of ${metrics.alpha.toFixed(2)}% with ${alphaSignificance.toLowerCase()} statistical significance.`,
    riskAnalysis: `Portfolio volatility of ${metrics.portfolioVolatility.toFixed(1)}% with tracking error of ${metrics.trackingError.toFixed(1)}%.`,
    ...
    alphaSignificance,
    betaStability,
    riskProfile: `Portfolio exhibits ${riskRating.toLowerCase()} risk with beta of ${metrics.beta.toFixed(2)}.`,
    ...
    factorExposure: `Market beta exposure of ${metrics.beta.toFixed(2)} with R-squared of ${metrics.rSquared.toFixed(2)}.`,
    ...
    benchmarkFit: `Benchmark correlation of ${metrics.correlation.toFixed(2)} indicates ${metrics.correlation > 0.8 ? 'strong' : metrics.correlation > 0.6 ? 'moderate' : 'weak'} benchmark fit.`,
...
function assessDataQuality(inputs: AlphaBetaInputs): number {
  let quality = 80;
...
function assessModelAccuracy(inputs: AlphaBetaInputs, rSquared: number): number {
  let accuracy = 70;
...
function calculateConfidenceLevel(inputs: AlphaBetaInputs, tStatistic: number, pValue: number): number {
  let confidence = 70;
...
function generateFactorDecomposition(inputs: AlphaBetaInputs, alpha: number, beta: number) {
  return [
    {
      factor: 'Market Beta',
      exposure: beta,
...
function generateStressTestScenarios(inputs: AlphaBetaInputs, alpha: number, beta: number, portfolioVolatility: number) {
  return [
    {
...
function generateOptimizationSuggestions(inputs: AlphaBetaInputs, alpha: number, beta: number, sharpeRatio: number, informationRatio: number) {
  const suggestions = [];
...
function generateRiskManagementSuggestions(inputs: AlphaBetaInputs, portfolioVolatility: number, trackingError: number, maxDrawdown: number) {
  const suggestions = [];
...
/**
 * Generate comprehensive alpha and beta analysis report
 */
export function generateAlphaBetaAnalysis(inputs: AlphaBetaInputs, outputs: AlphaBetaOutputs): string {
...
**Performance Rating**: ${outputs.analysis.performanceRating}
...
- **Systematic Risk**: ${outputs.systematicRisk.toFixed(1)}%
- **Idiosyncratic Risk**: ${outputs.idiosyncraticRisk.toFixed(1)}%

## Statistical Analysis
- **T-Statistic**: ${outputs.tStatistic.toFixed(2)}
- **P-Value**: ${outputs.pValue.toFixed(3)}
- **Alpha Significance**: ${outputs.analysis.alphaSignificance}
- **Confidence Interval**: [${outputs.confidenceInterval.lower.toFixed(2)}%, ${outputs.confidenceInterval.upper.toFixed(2)}%]
...
- **Expected Beta**: ${outputs.analysis.expectedBeta.toFixed(2)}

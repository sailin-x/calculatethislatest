import { day_tradingCalculatorInputs, day_tradingCalculatorOutputs, day_tradingCalculatorMetrics, day_tradingCalculatorAnalysis } from './types';

// Calculate total equity offered as percentage
export function calculateTotalEquityOffered(inputs: day_tradingCalculatorInputs): number {
  return inputs.equityPercentageOffered;
}

// Calculate price per share based on valuation and equity offered
export function calculatePricePerShare(inputs: day_tradingCalculatorInputs): number {
...
export function calculateSharesOffered(inputs: day_tradingCalculatorInputs): number {
...
export function calculateFundingProgress(inputs: day_tradingCalculatorInputs): number {
  return (inputs.currentFunding / inputs.totalFundingGoal) * 100;
}

// Calculate remaining funding needed
export function calculateRemainingFundingNeeded(inputs: day_tradingCalculatorInputs): number {
  return Math.max(0, inputs.totalFundingGoal - inputs.currentFunding);
}

// Calculate average investment per investor
export function calculateAverageInvestmentPerInvestor(inputs: day_tradingCalculatorInputs): number {
  return inputs.numberOfInvestors > 0 ? inputs.currentFunding / inputs.numberOfInvestors : 0;
}

// Calculate total fees
export function calculateTotalFees(inputs: day_tradingCalculatorInputs): number {
  return inputs.platformFees + inputs.legalFees + inputs.marketingFees;
}

// Calculate net proceeds after fees
export function calculateNetProceeds(inputs: day_tradingCalculatorInputs): number {
  return inputs.currentFunding - calculateTotalFees(inputs);
}

// Calculate post-money valuation
export function calculatePostMoneyValuation(inputs: day_tradingCalculatorInputs): number {
  return inputs.valuation + inputs.currentFunding;
}

// Calculate ownership dilution
export function calculateOwnershipDilution(inputs: day_tradingCalculatorInputs): number {
...
export function calculateBreakEvenInvestors(inputs: day_tradingCalculatorInputs): number {
...
export function calculateProjectedROI(inputs: day_tradingCalculatorInputs): number {
  return inputs.expectedReturn;
}

// Calculate risk-adjusted return
export function calculateRiskAdjustedReturn(inputs: day_tradingCalculatorInputs): number {
...
export function calculateSuccessProbability(inputs: day_tradingCalculatorInputs): number {
...
export function calculateResult(inputs: day_tradingCalculatorInputs): number {
  return calculateFundingProgress(inputs);
}

// Generate analysis based on inputs and metrics
export function generateAnalysis(inputs: day_tradingCalculatorInputs, metrics: day_tradingCalculatorMetrics): day_tradingCalculatorAnalysis {
  const progress = calculateFundingProgress(inputs);

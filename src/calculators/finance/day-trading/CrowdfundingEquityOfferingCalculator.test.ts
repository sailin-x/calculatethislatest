...
import { validateday_tradingCalculatorInputs } from './validation';
...
describe('Crowdfunding Equity Offering Calculator', () => {
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateday_tradingCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates funding goal cannot be zero', () => {
      const invalidInputs = { ...mockInputs, totalFundingGoal: 0 };
      const result = validateday_tradingCalculatorInputs(invalidInputs);

...
import { validateEthereum2StakingCalculatorInputs } from './validation';

describe('Ethereum 2 Staking Calculator', () => {
  const mockInputs = {
    inputValue: 10,
    multiplier: 5
  };
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateEthereum2StakingCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates negative numbers', () => {
      const invalidInputs = { ...mockInputs, inputValue: -5 };
      const result = validateEthereum2StakingCalculatorInputs(invalidInputs);

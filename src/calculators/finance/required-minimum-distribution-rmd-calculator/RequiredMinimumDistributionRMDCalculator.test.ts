...
describe('Required Minimum Distribution (RMD) Calculator', () => {
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateRequiredMinimumDistributionRmdCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates negative numbers', () => {
      const invalidInputs = { ...mockInputs, inputValue: -5 };
      const result = validateRequiredMinimumDistributionRmdCalculatorInputs(invalidInputs);

...
import { calculateFuturesTrading } from './formulas';
import { validateFuturesTradingInputs } from './validation';
import { validateField } from './quickValidation';

export default function FuturesTradingCalculator() {
  const [inputs, setInputs] = useState<FuturesTradingInputs>({
    ...
    // Technical Analysis
    technicalIndicators: [
      { name: 'RSI', signal: 'buy', value: 30 },
      { name: 'MACD', signal: 'buy', value: 0.5 },
      { name: 'Moving Average', signal: 'neutral', value: 100 }
    ],
  ...
  const [results, setResults] = useState<FuturesTradingOutputs | null>(null);
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [warnings, setWarnings] = useState<Record<string, string>>({});

  useEffect(() => {
    const validation = validateFuturesTradingInputs(inputs);
    if (validation.isValid) {
      try {
        const calculatedResults = calculateFuturesTrading(inputs);
        setResults(calculatedResults);
        setErrors({});
      } catch (error) {
        setErrors({ calculation: 'Error calculating futures trading metrics' });
      }
    } else {
  ...
  const handleInputChange = (field: keyof FuturesTradingInputs, value: any) => {
    const newInputs = { ...inputs, [field]: value };
    setInputs(newInputs);

    // Auto-calculate related fields
    if (field === 'currentPrice' && inputs.entryPrice > 0) {
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-bold">Futures Trading Calculator</h1>
        <p className="text-muted-foreground">
          Advanced futures trading analysis with risk management and strategy optimization
        </p>
      </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  />
                  {errors.targetPrice && <p className="text-sm text-red-500">{errors.targetPrice}</p>}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className={errors.stopLossPrice ? 'border-red-500' : ''}
                  />
                  {errors.stopLossPrice && <p className="text-sm text-red-500">{errors.stopLossPrice}</p>}
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  />
                  {errors.accountBalance && <p className="text-sm text-red-500">{errors.accountBalance}</p>}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    type="number"
                    step="100"
                    value={inputs.initialMargin}
                    onChange={(e) => handleInputChange('initialMargin', parseFloat(e.target.value) || 0)}
                    className={errors.initialMargin ? 'border-red-500' : ''}
                  />
                  {errors.initialMargin && <p className="text-sm text-red-500">{errors.initialMargin}</p>}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => handleInputChange('maintenanceMargin', parseFloat(e.target.value) || 0)}
                    className={errors.maintenanceMargin ? 'border-red-500' : ''}
                  />
                  {errors.maintenanceMargin && <p className="text-sm text-red-500">{errors.maintenanceMargin}</p>}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  />
                  {errors.leverage && <p className="text-sm text-red-500">{errors.leverage}</p>}
                </div>
              </CardContent>
            </Card>
          </div>

...
import { validate529CollegeSavingsPlanCalculatorInputs } from './validation';

describe('529CollegeSavingsPlanCalculator Calculator', () => {
  const mockInputs = {
    primaryInput: 100,
    secondaryInput: 50,
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validate529CollegeSavingsPlanCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates required primary input', () => {
      const invalidInputs = { ...mockInputs, primaryInput: 0 };
      const result = validate529CollegeSavingsPlanCalculatorInputs(invalidInputs);

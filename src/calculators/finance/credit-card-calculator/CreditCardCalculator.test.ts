...
import { validateCreditCardCalculatorInputs } from './validation';
...
describe('Credit Card Calculator', () => {
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateCreditCardCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates negative numbers', () => {
      const invalidInputs = { ...mockInputs, inputValue: -5 };
      const result = validateCreditCardCalculatorInputs(invalidInputs);

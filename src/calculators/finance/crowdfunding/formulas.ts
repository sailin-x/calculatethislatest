import { CrowdfundingInputs, CrowdfundingOutputs, CrowdfundingMetrics, CrowdfundingAnalysis } from './types';

// Calculate total equity offered as percentage
export function calculateTotalEquityOffered(inputs: CrowdfundingInputs): number {
  return inputs.equityPercentageOffered;
}

// Calculate price per share based on valuation and equity offered
export function calculatePricePerShare(inputs: CrowdfundingInputs): number {
...
export function calculateSharesOffered(inputs: CrowdfundingInputs): number {
...
export function calculateFundingProgress(inputs: CrowdfundingInputs): number {
  return (inputs.currentFunding / inputs.totalFundingGoal) * 100;
}

// Calculate remaining funding needed
export function calculateRemainingFundingNeeded(inputs: CrowdfundingInputs): number {
  return Math.max(0, inputs.totalFundingGoal - inputs.currentFunding);
}

// Calculate average investment per investor
export function calculateAverageInvestmentPerInvestor(inputs: CrowdfundingInputs): number {
  return inputs.numberOfInvestors > 0 ? inputs.currentFunding / inputs.numberOfInvestors : 0;
}

// Calculate total fees
export function calculateTotalFees(inputs: CrowdfundingInputs): number {
  return inputs.platformFees + inputs.legalFees + inputs.marketingFees;
}

// Calculate net proceeds after fees
export function calculateNetProceeds(inputs: CrowdfundingInputs): number {
  return inputs.currentFunding - calculateTotalFees(inputs);
}

// Calculate post-money valuation
export function calculatePostMoneyValuation(inputs: CrowdfundingInputs): number {
  return inputs.valuation + inputs.currentFunding;
}

// Calculate ownership dilution
export function calculateOwnershipDilution(inputs: CrowdfundingInputs): number {
...
export function calculateBreakEvenInvestors(inputs: CrowdfundingInputs): number {
...
export function calculateProjectedROI(inputs: CrowdfundingInputs): number {
  return inputs.expectedReturn;
}

// Calculate risk-adjusted return
export function calculateRiskAdjustedReturn(inputs: CrowdfundingInputs): number {
...
export function calculateSuccessProbability(inputs: CrowdfundingInputs): number {
...
export function calculateResult(inputs: CrowdfundingInputs): number {
  return calculateFundingProgress(inputs);
}

// Generate analysis based on inputs and metrics
export function generateAnalysis(inputs: CrowdfundingInputs, metrics: CrowdfundingMetrics): CrowdfundingAnalysis {
  const progress = calculateFundingProgress(inputs);

...
import { validateConstructionAccidentClaimsCalculatorInputs } from './validation';

describe('ConstructionAccidentClaimsCalculator Calculator', () => {
  const mockInputs = {
    primaryInput: 100,
    secondaryInput: 50,
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateConstructionAccidentClaimsCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates required primary input', () => {
      const invalidInputs = { ...mockInputs, primaryInput: 0 };
      const result = validateConstructionAccidentClaimsCalculatorInputs(invalidInputs);

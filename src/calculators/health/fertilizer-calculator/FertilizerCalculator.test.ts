...
import { validateFertilizerCalculatorInputs } from './validation';
...
describe('Fertilizer Calculator', () => {
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateFertilizerCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates negative numbers', () => {
      const invalidInputs = { ...mockInputs, inputValue: -5 };
      const result = validateFertilizerCalculatorInputs(invalidInputs);

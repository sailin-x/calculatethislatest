...
import { validateSleepCalculatorInputs } from './validation';
...
describe('Sleep Calculator', () => {
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateSleepCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates negative numbers', () => {
      const invalidInputs = { ...mockInputs, inputValue: -5 };
      const result = validateSleepCalculatorInputs(invalidInputs);

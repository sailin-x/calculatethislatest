...
import { validateOmega3CalculatorInputs } from './validation';
...
describe('Omega 3 Calculator', () => {
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateOmega3CalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates negative numbers', () => {
      const invalidInputs = { ...mockInputs, inputValue: -5 };
      const result = validateOmega3CalculatorInputs(invalidInputs);

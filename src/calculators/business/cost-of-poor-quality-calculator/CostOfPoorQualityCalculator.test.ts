...
describe('Cost of Poor Quality Calculator', () => {
  const mockInputs = {
    inputValue: 10,
    multiplier: 5
  };
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateCostOfPoorQualityCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates negative numbers', () => {
      const invalidInputs = { ...mockInputs, inputValue: -5 };
      const result = validateCostOfPoorQualityCalculatorInputs(invalidInputs);

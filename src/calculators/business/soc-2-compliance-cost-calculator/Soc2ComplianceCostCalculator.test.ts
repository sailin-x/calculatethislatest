...
import { validateSoc2ComplianceCostCalculatorInputs } from './validation';

describe('SOC 2 Compliance Cost Calculator', () => {
  const mockInputs = {
    inputValue: 10,
    multiplier: 5
  };
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateSoc2ComplianceCostCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates negative numbers', () => {
      const invalidInputs = { ...mockInputs, inputValue: -5 };
      const result = validateSoc2ComplianceCostCalculatorInputs(invalidInputs);

...
import { validatePublicprivatePartnershipP3RoiCalculatorInputs } from './validation';

describe('PublicprivatePartnershipP3RoiCalculator Calculator', () => {
  const mockInputs = {
    primaryInput: 100,
    secondaryInput: 50,
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validatePublicprivatePartnershipP3RoiCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates required primary input', () => {
      const invalidInputs = { ...mockInputs, primaryInput: 0 };
      const result = validatePublicprivatePartnershipP3RoiCalculatorInputs(invalidInputs);

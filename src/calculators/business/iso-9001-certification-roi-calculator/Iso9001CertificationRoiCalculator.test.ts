...
import { validateIso9001CertificationRoiCalculatorInputs } from './validation';

describe('Iso9001CertificationRoiCalculator Calculator', () => {
  const mockInputs = {
    primaryInput: 100,
    secondaryInput: 50,
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateIso9001CertificationRoiCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates required primary input', () => {
      const invalidInputs = { ...mockInputs, primaryInput: 0 };
      const result = validateIso9001CertificationRoiCalculatorInputs(invalidInputs);

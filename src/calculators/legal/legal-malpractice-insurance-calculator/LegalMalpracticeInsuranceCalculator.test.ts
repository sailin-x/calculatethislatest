...
import { validateLegalMalpracticeInsuranceCalculatorInputs } from './validation';

describe('LegalMalpracticeInsuranceCalculator Calculator', () => {
  const mockInputs = {
    primaryInput: 100,
    secondaryInput: 50,
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateLegalMalpracticeInsuranceCalculatorInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates required primary input', () => {
      const invalidInputs = { ...mockInputs, primaryInput: 0 };
      const result = validateLegalMalpracticeInsuranceCalculatorInputs(invalidInputs);

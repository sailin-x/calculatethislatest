...
describe('Merger & Acquisition (M&A) Divestiture Valuation', () => {
  ...
  describe('Validation', () => {
    it('validates correct inputs', () => {
      const result = validateMergerAcquisitionDivestitureValuationInputs(mockInputs);
      expect(result.length).toBe(0);
    });

    it('validates negative numbers', () => {
      const invalidInputs = { ...mockInputs, inputValue: -5 };
      const result = validateMergerAcquisitionDivestitureValuationInputs(invalidInputs);
